...
(function() {
  ...
  const isGoogleMapsFullyLoaded = () => {
    try {
      ...
      const requiredClasses = [
        'Map', 'Circle', 'Geocoder', 'InfoWindow', 'LatLngBounds'
      ];
      ...
      // Check for new marker system
      if (!window.google.maps.marker || !window.google.maps.marker.AdvancedMarkerElement) {
        console.log('Google Maps: AdvancedMarkerElement not available, falling back to Marker');
        if (!window.google.maps.Marker) {
          console.log('Google Maps: Marker also not available');
          return false;
        }
      }
  ...
  // Helper function to create markers (uses AdvancedMarkerElement if available, fallback to Marker)
  const createMarker = (options) => {
    if (window.google.maps.marker && window.google.maps.marker.AdvancedMarkerElement) {
      // Use modern AdvancedMarkerElement
      const pinElement = new window.google.maps.marker.PinElement({
        background: options.iconColor || '#2563eb',
        borderColor: '#ffffff',
        glyphColor: '#ffffff',
  ...
  const useGoogleMaps = () => {
        ...
        for (const office of offices) {
          ...
          if (coords) {
            try {
              ...
              // Add click listener (different for AdvancedMarkerElement vs Marker)
    ...
    useEffect(() => {
      ...
      // Use proper async loading with marker library
      script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyBaTFTW_OSfqCt93_P7rcjlXhU1RInOkj0&libraries=geometry,marker&loading=async&callback=${uniqueCallback}`;
      script.async = true;
      script.defer = true;
