...
export const fetchOfficesFromWebflow = async (): Promise<Office[]> => {
  try {
    console.log('Fetching offices from Webflow CMS...');
    
    const response = await fetch(`${WEBFLOW_BASE_URL}/v2/collections/${COLLECTION_ID}/items`, {
      headers: {
    ...
    const offices: Office[] = data.items
      ...
      .map((item: WebflowOffice, index: number) => ({
        ...
        lat: item.fieldData.latitude || 0,
        lng: item.fieldData.longitude || 0,
    ...
    console.error('Error fetching offices from Webflow:', error);
    throw error;
  }
};

// Fallback to get site collections (for debugging)
export const getWebflowCollections = async (siteId: string) => {
  try {
    const response = await fetch(`${WEBFLOW_BASE_URL}/v2/sites/${siteId}/collections`, {
      headers: {
        'Authorization': `Bearer ${WEBFLOW_API_KEY}`,
        'accept': 'application/json',
      },
